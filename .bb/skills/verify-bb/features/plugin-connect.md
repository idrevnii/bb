...
| Gate without tickets | Restart the local cloud gate with `--var TUNNEL_TICKETS:off` (or stop the web worker so tickets can't be minted), then reconnect the test tunnel; downgrade the same store to a build from before bb account. | Server log shows `the gate refused this bb's tunnel ticket (HTTP 401)` or `can't get a tunnel ticket`, then `tunnel connected with the server credential`; the older build starts paired from Connect's `credential` KV key. |
