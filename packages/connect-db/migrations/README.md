...
Migration `0006_machine_session_presence.sql` adds `machine.session_seen_at`. The gate
updates it only when a daemon with a valid machine credential reports an
accepted bb server session. Apply 0006 before deploying the gate or dashboard
