...
async function dial(
  host: string,
  bearer: string,
  flag: { TUNNEL_TICKETS?: string } = { TUNNEL_TICKETS: "on" },
) {
...
describe("tunnel dial while TUNNEL_TICKETS is not on", () => {
  it.each([
    ["off", { TUNNEL_TICKETS: "off" }],
    ["unset", {}],
  ])("refuses a valid ticket with 401 when %s", async (_label, flag) => {
    const ticket = await ticketFor("srv-primary");
  ...
  it("still accepts the raw server credential", async () => {
    const { response, forwarded } = await dial(
      "sawyer.getbb.app",
      RAW_CREDENTIAL,
      { TUNNEL_TICKETS: "off" },
