...
`login.start`, `login.poll`, `login.cancel`, `redeemCode`, and `signOut`.
Only the connect plugin may call the two `bb-account.v1.*` methods.
...
- `connectCredential` returns `{credential: {baseUrl, serverUrl, serverId,
  credential} | null}`. Connect uses it to dial the tunnel without a ticket
  when getbb.app or the gate can't use tickets, and to keep its copy for
  older builds current. `signOut` returns `{revocation, status}`, where
