...
import { ConnectTunnel, TICKET_PAUSE_MS } from "./tunnel.js";
...
describe("ConnectTunnel socket lifecycle", () => {
  ...
  it("redials with the server credential after the gate refuses a ticket, and skips tickets for 30 minutes", async () => {
    vi.useFakeTimers();
    const { fakeHost, mint, tunnel } = createTunnelFixture();
    ...
    try {
      ...
      const url = new URL(fakeWebSockets.urls[1]!);
      expect(`${url.origin}${url.pathname}`).toBe(
        "wss://sawyer.getbb.app/__tunnel",
      );
      expect(url.searchParams.get("v")).toBe("1");

      fakeWebSockets.instances[1]!.emit("close", 1006, Buffer.from(""));
      await vi.advanceTimersByTimeAsync(
        tunnel.status().nextRetryAt! - Date.now(),
      );
      expect(mint).toHaveBeenCalledTimes(1);
      expect(fakeWebSockets.options[2]?.headers).toEqual({
        authorization: "Bearer bbcred_server",
      });

      vi.setSystemTime(Date.now() + TICKET_PAUSE_MS);
      fakeWebSockets.instances[2]!.emit("close", 1006, Buffer.from(""));
      await vi.advanceTimersByTimeAsync(
        tunnel.status().nextRetryAt! - Date.now(),
      );
      expect(mint).toHaveBeenCalledTimes(2);
      expect(fakeWebSockets.options[3]?.headers).toEqual({
        authorization: "Bearer bbtkt_1",
      });
    } finally {
      tunnel.stop();
  ...
  it("reports a refused server credential and backs off instead of dialing in a loop", async () => {
    vi.useFakeTimers();
    const { fakeHost, tunnel } = createTunnelFixture(async () => {
  ...
  it("does not fall back when the gate says the server isn't paired", async () => {
    vi.useFakeTimers();
    const { fakeHost, mint, tunnel } = createTunnelFixture();
  ...
  it("dials with the server credential when a ticket can't be minted", async () => {
    vi.useFakeTimers();
    const { fakeHost, mint, tunnel } = createTunnelFixture(async () => {
      throw new Error("getbb.app returned HTTP 503 for a tunnel ticket");
    });
  ...
  it("backs off when neither a ticket nor the server credential is available", async () => {
    vi.useFakeTimers();
    let failures = 1;
    const { fakeHost, mint, tunnel } = createTunnelFixture(
      async () => {
