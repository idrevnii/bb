...
describe("connect on top of bb account", () => {
  ...
  it("falls back to the raw credential when the gate refuses tickets, as the pre-ticket gate does", async () => {
    stub.acceptTunnelTickets = false;
    await loadBoth();
    await signInAccount();
    startTunnel();
    ...
    expect(stub.tunnelDials[0]?.socket).toBeNull();
    expect(stub.tunnelDials[1]?.authorization).toBe(`Bearer ${credential}`);
    expect(await accountState()).toBe("signed-in");
  });

  it("falls back to the raw credential when the ticket endpoint answers 401 but the pairing is still valid", async () => {
    stub.route("POST", "/api/connect/tunnel-ticket", () => ({
      status: 401,
  ...
  it("falls back to the raw credential when getbb.app can't mint a ticket", async () => {
    stub.route("POST", "/api/connect/tunnel-ticket", () => ({
      status: 500,
      body: { error: "internal" },
    }));
    ...
    await waitForConnected(1);
    expect(stub.mintedTickets).toEqual([]);
    expect(stub.tunnelDials[0]?.authorization).toBe(
      `Bearer ${await storedCredential()}`,
    );
  });
