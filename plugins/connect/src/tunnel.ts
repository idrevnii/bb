...
export const TICKET_PAUSE_MS = 30 * 60 * 1000;

interface DialAuth {
  kind: "ticket" | "credential";
  bearer: string;
  tunnelUrl: string;
}
...
export class ConnectTunnel {
  ...
  private async mintAndDial(
    ...
    if (Date.now() >= this.ticketsPausedUntil) {
      ...
      try {
        ...
        if (error instanceof AccountUnavailableError) {
          this.lastError = `can't get a tunnel ticket — ${error.message}`;
      ...
      if (ticket !== null) {
        this.dial(
          identity,
          { kind: "ticket", bearer: ticket.ticket, tunnelUrl: ticket.tunnelUrl },
          epoch,
        );
        return;
      }
    }
    await this.dialWithCredential(identity, epoch);
  }
  ...
  private async dialWithCredential(
    ...
    this.dial(
      ...
      {
        kind: "credential",
        bearer: shared.credential,
        tunnelUrl: `${identity.serverUrl.replace(/^http/u, "ws")}/__tunnel`,
      },
      epoch,
    );
  }
  ...
  private pauseTickets(reason: string): void {
    this.ticketsPausedUntil = Date.now() + TICKET_PAUSE_MS;
    this.options.log.warn(
      `${reason}; dialing with the server credential for the next ${TICKET_PAUSE_MS / 60_000} minutes`,
    );
  }

  private dial(identity: ConnectIdentity, auth: DialAuth, epoch: number): void {
    const tunnelUrl = tunnelDialUrl(auth.tunnelUrl);
    const using = auth.kind === "ticket" ? "a ticket" : "the server credential";
    this.options.log.info(
      `tunnel connecting to ${tunnelUrl} with ${using} (origin ${this.options.getLoopbackBaseUrl()})`,
    );
    ...
    tunnel.on("open", () => {
      ...
      this.lastError = null;
      this.nextRetryAt = null;
      this.options.log.info(`tunnel connected with ${using}`);
    ...
    tunnel.on("unexpected-response", (_req, res) => {
      ...
      if (statusCode === 401 && auth.kind === "ticket") {
        this.lastError = `the gate refused this bb's tunnel ticket (HTTP ${statusCode})`;
        this.pauseTickets(this.lastError);
      } else {
        this.lastError =
          statusCode === 401 || statusCode === 403
            ? `the gate refused this bb's ${auth.kind === "ticket" ? "tunnel ticket" : "server credential"} (HTTP ${statusCode})`
