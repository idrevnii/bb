...
Remote access uses the bb account plugin. Sign in from Settings → Remote access, Settings → bb account, or `bb account login`. A pairing code from the getbb.app dashboard also works: paste it in Settings or run `bb connect --code <code>`. Remote access starts as soon as the account is signed in. Before each connection, the plugin gets a five-minute ticket from getbb.app. If getbb.app or the gate can't use tickets, it connects with the server credential instead. The plugin reconnects after a drop.
